...
public class NoPreloadViewPager extends ViewGroup {
    private String TAG = "NoPreloadViewPager";
    private static final boolean DEBUG = true;

    private static final boolean USE_CACHE = false;

    private boolean isScroll = true;// 是否允许左右滑动  默认可以
...
//            mAdapter.registerDataSetObserver(mObserver);
    ...
    private void calculatePageOffsets() {
        ...
        mLastOffset = offset - 1;

        Log.v(TAG,"calculatePageOffsets----mLastOffset="+mLastOffset);
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            case MotionEvent.ACTION_MOVE: {
                ...
                if (DEBUG) Log.v(TAG, "Moved x to " + x + "," + y + " d+iff=" + xDiff + "," + yDiff);
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        if (mFakeDragging) {
            ...
            Log.v(TAG,"onTouch----mFakeDragging----"+ev.getAction()+"----mIsBeingDragged="+mIsBeingDragged);
            return true;
        }
        ...
        if (ev.getAction() == MotionEvent.ACTION_DOWN && ev.getEdgeFlags() != 0) {
            ...
            // descendants.
            Log.v(TAG,"onTouch----边缘----"+ev.getAction()+"----mIsBeingDragged="+mIsBeingDragged);
            return false;
        }

        if (mAdapter == null || mAdapter.getCount() == 0) {
            // Nothing to present or scroll; nothing to touch.
            Log.v(TAG,"onTouch----没数据----"+ev.getAction()+"----mIsBeingDragged="+mIsBeingDragged);
            return false;
        }
        ...
        Log.v(TAG,"onTouch----"+action+"----mIsBeingDragged="+mIsBeingDragged);
        return true;
    }
